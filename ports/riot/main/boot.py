...
if 1:  # test `voucher` module
    ...
    def wip():
        ...
        vrq.set(ATTR_ASSERTION, ASSERTION_PROXIMITY) \
           ...
           .set(ATTR_SERIAL_NUMBER, '00-D0-E5-F2-00-02') \
           .sign(KEY_PEM_F2_00_02, SA_ES256) \
           .dump()

        # test_assert('voucher.{sign,validate} - vrq sample F2_00_02 via pubkey',
        #     voucher.validate(bs_vrq_sample, bs_device_crt_f2))  # << DEVICE_CRT_F2_00_02
        #==
        # vrq.validate(DEVICE_CRT_F2_00_02)  # with PEM

        # test_assert('voucher.{sign,validate} - vrq sample F2_00_02 via privkey',
        #     voucher.validate(bs_vrq_sample, bs_key_pem_f2))  # << KEY_PEM_F2_00_02
        #==
        vrq.validate(KEY_PEM_F2_00_02)  # with PEM
