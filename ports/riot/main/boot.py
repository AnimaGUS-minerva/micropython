...
print('debug_demo():', voucher.debug_demo())  # NEW
...
if 1:  # test `voucher` module
    tpl = vd_test_ffi()
    test_assert_eq('vd_test_ffi',
        tpl, (42, False, None, True, False, b'\xa0\xb1\xc2\xd3\xe4\xf5', False))
    # print(tpl)

    init_psa_crypto()

    bs_jada = vd_get_vch_jada()
    test_assert_eq('vd_get_vch_jada', len(bs_jada), 328)
    # print(len(bs_jada), bs_jada, list(bs_jada))

    bs_f2 = vd_get_vch_F2_00_02()
    test_assert_eq('vd_get_vch_F2_00_02', len(bs_f2), 771)
    # print(len(bs_f2), bs_f2)

    bs_pem_f2 = vd_get_masa_pem_F2_00_02()
    test_assert_eq('vd_get_masa_pem_F2_00_02', len(bs_pem_f2), 684)
    ...
    test_assert('vd_validate - jada', vd_validate(bs_jada))
    test_assert('vd_validate - F2_00_02', vd_validate(bs_f2, bs_pem_f2))

    # vd_dump(bs_jada)  # TODO handle panic on decode failure

    #

    bs_key_pem_f2 = vd_get_key_pem_F2_00_02()
    test_assert_eq('vd_get_key_pem_F2_00_02', len(bs_key_pem_f2), 227)

    bs_device_crt_f2 = vd_get_device_crt_F2_00_02()
    test_assert_eq('vd_get_device_crt_F2_00_02', len(bs_device_crt_f2), 644)

    #

    bs_vrq_sample = vd_get_vrq_F2_00_02()
    test_assert_eq('bs_vrq_sample (with bare signature)', len(bs_vrq_sample), 622)

    test_assert('vd_{sign,validate} - vrq sample F2_00_02 via pubkey',
        vd_validate(bs_vrq_sample, bs_device_crt_f2))
    test_assert('vd_{sign,validate} - vrq sample F2_00_02 via privkey',
        vd_validate(bs_vrq_sample, bs_key_pem_f2))

    #

    bs_vrq = vd_create_vrq_F2_00_02()
    # print('bs_vrq', list(bs_vrq))
    test_assert_eq('bs_vrq', len(bs_vrq), 555)

    test_assert('vd_{sign,validate} - validating an unsigned voucher should fail',
        not vd_validate(bs_vrq, bs_device_crt_f2))

    bs_vrq_signed = vd_sign(bs_vrq, bs_key_pem_f2, SA_ES256)
    # print('bs_vrq_signed', list(bs_vrq_signed))
    test_assert_eq('bs_vrq_signed (with asn1 signature)', len(bs_vrq_signed), 630)

    test_assert('vd_{sign,validate} - vrq F2_00_02 via pubkey',
        vd_validate(bs_vrq_signed, bs_device_crt_f2))
    test_assert('vd_{sign,validate} - vrq F2_00_02 via privkey',
        vd_validate(bs_vrq_signed, bs_key_pem_f2))
    ...
    def wip():
        ...
        KEY_PEM_F2_00_02 = vd_get_key_pem_F2_00_02()        # debug, privkey
        DEVICE_CRT_F2_00_02 = vd_get_device_crt_F2_00_02()  # debug, pubkey
        ...
        test_assert('vch_jada.validate() - without PEM, `signer_cert` is used instead',
            from_cbor(vd_get_vch_jada()).validate())
        test_assert('vch_f2.validate() - without PEM, should fail',
            not from_cbor(vd_get_vch_F2_00_02()).validate())
        test_assert('vrq_f2.validate() - without PEM, should fail',
            not from_cbor(vd_get_vrq_F2_00_02()).validate())
        ...
        obj_vch_jada = from_cbor(vd_get_vch_jada())
        obj_vch_f2 = from_cbor(vd_get_vch_F2_00_02())
        obj_vrq_f2 = from_cbor(vd_get_vrq_F2_00_02())
        ...
        test_assert_eq('to_cbor() - obj_vch_jada', obj_vch_jada.to_cbor(), vd_get_vch_jada())
        test_assert_eq('to_cbor() - obj_vch_f2', obj_vch_f2.to_cbor(), vd_get_vch_F2_00_02())
        test_assert_eq('to_cbor() - obj_vrq_f2', obj_vrq_f2.to_cbor(), vd_get_vrq_F2_00_02())
        ...
        vch = voucher.from_cbor(vd_get_vch_jada())
        vch.debug_dump()
        ...
        vch = voucher.from_cbor(vd_get_vch_jada())#.debug_dump()
        ...
        v = from_cbor(vd_get_vch_F2_00_02())
        ...
        v = from_cbor(vd_get_vch_jada())#.debug_dump()
        ...
        v = from_cbor(vd_get_vch_F2_00_02())#.debug_dump()
        ...
        v = from_cbor(vd_get_vrq_F2_00_02())#.debug_dump()
